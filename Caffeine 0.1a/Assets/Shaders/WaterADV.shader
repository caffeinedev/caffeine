...
Shader "Water/WaterADV" {
    Properties {
        ...
        _node_4625 ("node_4625", Color) = (0.7647059,0.8831642,1,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                float3 emissive = (_node_965.rgb*max(attenuation,_node_4625.rgb));
                float3 node_8503 = ((normalDirection*lightDirection)*-1.5+0.5);
                float node_3537 = 2.0;
                float node_3865 = 2.0;
                float3 finalColor = emissive + saturate((1.0-(1.0-(floor(pow(1.0-max(0,dot(node_8503, viewDirection)),_Fresnel2Intensity) * node_3537) / (node_3537 - 1)*_FresnelTint1.rgb*0.5))*(1.0-saturate((floor(pow(1.0-max(0,dot(node_8503, viewDirection)),_Frenel2Intensity) * node_3865) / (node_3865 - 1)*_FresnelTint2.rgb)))));
...
////// Lighting:
                ...
                float node_3537 = 2.0;
                float node_3865 = 2.0;
                float3 finalColor = saturate((1.0-(1.0-(floor(pow(1.0-max(0,dot(node_8503, viewDirection)),_Fresnel2Intensity) * node_3537) / (node_3537 - 1)*_FresnelTint1.rgb*0.5))*(1.0-saturate((floor(pow(1.0-max(0,dot(node_8503, viewDirection)),_Frenel2Intensity) * node_3865) / (node_3865 - 1)*_FresnelTint2.rgb)))));
