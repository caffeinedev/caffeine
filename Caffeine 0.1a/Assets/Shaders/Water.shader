...
////// Emissive:
                ...
                float node_3537 = 2.0;
                float node_3865 = 3.0;
                float3 node_8648 = saturate((1.0-(1.0-(floor(pow(1.0-max(0,dot((normalDirection*-0.5+1.0), viewDirection)),_Fresnel2Intensity) * node_3537) / (node_3537 - 1)*_FresnelTint1.rgb*0.5))*(1.0-saturate((floor(pow(1.0-max(0,dot((normalDirection*-1.0+1.0), viewDirection)),_Frenel2Intensity) * node_3865) / (node_3865 - 1)*_FresnelTint2.rgb)))));
                float3 finalColor = emissive + node_8648;
