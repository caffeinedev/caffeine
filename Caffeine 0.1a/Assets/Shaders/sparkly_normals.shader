...
////// Emissive:
                ...
                float node_5434 = 2.0;
                float3 finalColor = emissive + (floor(clamp(clamp(pow(1.0-max(0,dot(normalDirection, viewDirection)),_fresnelintensity),0,0.9),0.1,0.5) * node_5434) / (node_5434 - 1)*_LightColor0.rgb*_GlossColor.rgb);
...
////// Lighting:
                float attenuation = LIGHT_ATTENUATION(i);
                float node_5434 = 2.0;
                float3 finalColor = (floor(clamp(clamp(pow(1.0-max(0,dot(normalDirection, viewDirection)),_fresnelintensity),0,0.9),0.1,0.5) * node_5434) / (node_5434 - 1)*_LightColor0.rgb*_GlossColor.rgb);
