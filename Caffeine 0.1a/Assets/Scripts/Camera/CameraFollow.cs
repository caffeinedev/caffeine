...
public class CameraFollow : MonoBehaviour 
{
	...
	void Awake ()
	{
		followTarget		= new GameObject("_followTarget").transform; //create empty gameObject as camera target, this will follow and rotate around the player
	...
	void Update ()
	{
		...
		// Get player input
		float h = (Input.GetAxis ("Horizontal") + Input.GetAxis ("AltHorizontal")) * inputRotationSpeed * Time.deltaTime;
		followTarget.RotateAround (target.position, Vector3.up, h);
